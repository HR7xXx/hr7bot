...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:HR7\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=555496535602:+55 54996535602\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Oii* @${num.split('@')[0]}\n*Boas-vindas ao servidor* *${mdata.subject}*\n\n*Esperamos que curta o servidor ;)*️️`
				...
				teks = `*Agradecemos por ter participado do servidor* @${num.split('@')[0]} *volte sempre!* 🐊`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					premium: '[❗] *ESTE COMANDO É SÓ PARA USUÁRIOS PREMIUM*',
					mod: '[❗] *ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD SONATA BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] *ESTE É UM RECURSO ESPECIAL PARA O HR7.*',
					ownerB: '[❗] *ESTE É UM RECURSO ESPECIAL PARA O HR7.*',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmão. Você não está registrado como amigo do HR7. Registre-se para fazer amizade com o bot Sonata por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar HR7|18\n\n──「 SONATA BOT 」──`,
			...
			const ownerNumber = ["555496535602@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"555496535602@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["555496535602@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["555496535602@s.whatsapp.net"]//ubah nomor lo
			const premium = ["555496535602@s.whatsapp.net","555497087266@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('[❗] *PSIU, VOCÊ É ADM E DEVERIA DAR EXEMPLO PARA OS OUTROS MEMBROS, PARE DE RACISMO!*')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`[❗] *ADEUS* ${sender.split("@")[0]} *VOCÊ SERÁ EXPULSO DO GRUPO EM 5 SEGUNDOS!*`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("[❗] *NO GRUPO NÃO PERMITIRMOS ATITUDES RACISTAS, QUE ISSO SIRVA DE EXEMPLO!*")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2")
		...
		if (isGroupAdmins) return reply('[🐊] *VOCÊ ESTÁ PERMITIDO POSTAR LINKS POIS É ADM!*')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`[❗] *LINK DETECTADO* ${sender.split("@")[0]} *VOCÊ SERÁ EXPULSO DO GRUPO EM 5 SEGUNDOS!*`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("Teu pai aquele gordozilla")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("Ah vai te arromba")
	}
		...
		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("Opa opa, me chamou gatinha? 😏")
	}
		...
		if (messagesC.includes("bot")){
			...
			if (messagesC.includes("motorola")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/motorola.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("rapaz")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/rapaz.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	        ...
	        if (messagesC.includes("chatuba")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/chatuba.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			if (messagesC.includes("kkkkk")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/oropuro.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
			if (messagesC.includes("gem")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/gemidao.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
			if (messagesC.includes("tiringa")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tiringa1.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
			if (messagesC.includes("maconheiro")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/sikera1.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			switch(command) {
				...
				case 'menu1':
					...
					reply(mess.wait)
					client.sendMessage(from, '[✅] *A MENSAGEM FOI ADICIONADA A LISTA COM SUCESSO!*' , text, { quoted: mek })
				    ...
				    hai = body.slice(8)
						...
						break
                   case 'saylist':
					teks = '[❗] *ESTÁ É A LISTA DE DIZERES* :\n'
				...
				case 'ssweb':
					...
					if (args.length < 1) return reply('[❗] *DIGITE A URL*')
					tekss = body.slice(7)
					reply(mess.wait)
		...
		case 'iri':
			...
			break
                ...
                case 'quotemaker':
					...
					break
				case 'galaxtext':
					if (args.length < 1) return reply('[❗] *O QUE VOCÊ PRECISA?*')
					teks = body.slice(12)
					if (teks.length > 8) return reply('[❗] *O TEXTO É LONGO, DIGITE ATÉ 8 CARÁCTERES*')
					reply(mess.wait)
                ...
                case 'tahta':
					...
					teks = body.slice(7)
					if (teks.length > 9) return reply('[❗] *O TEXTO É LONGO, DIGITE ATÉ 9 CARÁCTERES*')
					reply(mess.wait)
				...
				case 'lovemake':
					if (args.length < 1) return reply('[❗] *DIGITE O TEXTO*')
					love = body.slice(10)
					if (love.length > 12) return reply('[❗] *O TEXTO É LONGO, DIGITE ATÉ 9 CARÁCTERES*')
					reply(mess.wait)
				...
				case 'thunder':
					if (args.length < 1) return reply('[❗] *DIGITE O TEXTO*')
					thun = body.slice(9)
					if (thun.length > 10) return reply('[❗] *O TEXTO É LONGO, DIGITE ATÉ 9 CARÁCTERES*')
					reply(mess.wait)
		...
		case 'tahta':
                tahta = `${body.slice(7)}`
                     if (args.length < 1) return reply('[❗] *DIGITE O TEXTO*')
                     if (args.length > 10) return reply('[❗] *MÍNIMO DE 10 LINHAS*')
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
                 sck = `「 *SNACK VIDEO DOWNLOADER* 」\n\n*• Formato:* ${anu.format}\n*• Tamanho:* ${anu.size}\n\n*ENVIANDO, POR FAVOR NÃO DE SPAM*`
				...
				case 'party':
					if (args.length < 1) return reply(mess.blank)
					part = body.slice(7)
					if (part.length > 20) return reply('[❗] *O TEXTO É LONGO, DIGITE ATÉ 20 CARÁCTERES*')
					reply(mess.wait)
				...
				case 'modoanime':
					...
					if (args.length < 1) return reply('[❗] *ON PARA ATIVAR E OFF PARA DESATIVAR!*')
					if ((args[0]) === 'on') {
						...
						reply('[❗] *ON PARA ATIVAR E OFF PARA DESATIVAR!*')
					}
					break
					case 'antiracismo':
					if (!isGroup) return reply(mess.only.group)
					...
					if (args.length < 1) return reply('[❗] *ON PARA ATIVAR E OFF PARA DESATIVAR!*')
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('[❗] *O MODO ANTI-RACISMO JÁ ESTÁ ATIVO!*')
						antiracismo.push(from)
						fs.writeFileSync('./database/json/antiracismo.json', JSON.stringify(antiracismo))
						reply(`\`\`\`[✅] *ATIVADO COM SUCESSO O MODO ANTI-RACISMO NO GRUPO*\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						antiracismo.splice(from, 1)
						fs.writeFileSync('./database/json/antiracismo.json', JSON.stringify(antiracismo))
						reply(`\`\`\`[✅] *MODO ANTI-RACISMO DESATIVADO COM SUCESSO NO GRUPO*\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('[❗] *ON PARA ATIVAR E OFF PARA DESATIVAR!*')
					}
					break
				...
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
					if (tels5.length > 10) return reply('[❗] *O TEXTO É LONGO, NO MÁXIMO ATÉ 10 CARÁCTERES*')
					reply(mess.wait)
				...
				case 'water':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
					if (tels.length > 15) return reply('[❗] *O TEXTO É LONGO, NO MÁXIMO ATÉ 20 CARÁCTERES*')
					reply(mess.wait)
                 ...
                 case 'nomegp':
		             ...
		             break
				case 'loli2':
					if (!isAnime) return reply('[❌] *VOCÊ DEVE ATIVAR O MODO ANIME PARA USAR ESTE COMANDO!*')
...
//@darkYT					
    ...
    break
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('[❗] *O TEXTO É LONGO, NO MÁXIMO ATÉ 9 CARÁCTERES*')
					reply(mess.wait)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                     const pesan = body.slice(5)
                      if (!isPremium) return reply(mess.only.premium)
                      if (pesan.length > 5000) return client.sendMessage(from, '[❗] *ATÉ 5,000 CARÁCTERES*', msgType.text, {quoted: mek})
           ...
           case 'playmp3':
            ...
            case 'smule':
					...
					if (anu.error) return reply(anu.error)
					teks = `*Título* : ${anu.title}\n\n Espere alguns minutos, o download de vídeos esta executando`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek, caption: '*AQUI*'})
					await limitAdd(sender) 	
					break  
				case 'ttp':
					if (args.length < 1) return reply('[❗] *CADÊ O TEXTO?*')
...
break
case 'tiran':
tujuh = fs.readFileSync('./assets/tiran.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'rojao':
...
break
case 'cr7':
tujuh = fs.readFileSync('./assets/bestaenjaulada.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'a':
tujuh = fs.readFileSync('./assets/a.mp3');
...
break
case 'cachorrinhofdp':
tujuh = fs.readFileSync('./assets/cachorrinfdp.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'm4':
...
break
case 'eobraia':
tujuh = fs.readFileSync('./assets/murilety.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'fame':
...
break
case 'esskeetit':
tujuh = fs.readFileSync('./assets/esskeetit.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'taste':
tujuh = fs.readFileSync('./assets/taste.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'yosoypablo':
tujuh = fs.readFileSync('./assets/yosoypablo.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'pegaladrao':
tujuh = fs.readFileSync('./assets/pegaladrao.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'banido':
tujuh = fs.readFileSync('./assets/ednaldo1.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'nzap':
tujuh = fs.readFileSync('./assets/nzap.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'cca':
tujuh = fs.readFileSync('./assets/cca.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ccc':
tujuh = fs.readFileSync('./assets/ccc.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ccg':
tujuh = fs.readFileSync('./assets/ccg.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'cch':
tujuh = fs.readFileSync('./assets/cch.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ccr':
tujuh = fs.readFileSync('./assets/ccr.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ccp':
tujuh = fs.readFileSync('./assets/ccp.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ccm':
tujuh = fs.readFileSync('./assets/ccm.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
				case 'hinata':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS SONATA BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @HR7 domina vida
          ...
          reply('Espere..')
			    ...
			    case 'owner':
                    ...
                    client.sendMessage(from, '[🐊] *CONTATO DO MEU DONO*',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('[❗] *VOCÊ NÃO É UM MEMBRO PREMIUM, ENTRE EM CONTATO COM O HR7!*' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT SONATA ✨* 」\n*│+ wa.me/5554996535602*╰──────*「 *HR7* 」*────*\n\n*_SE QUER SER ADMIN DO BOT SONATA_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*CANAL DO HR7:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* HR7\n*YOUTUBE:* bit.ly/SONATABOT\n*WPP:* wa.me/+5554996535602\n*INSTA:* @sonatabot\n\n\n;)️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
